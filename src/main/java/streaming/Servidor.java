...
public class Servidor extends JFrame implements ActionListener, Runnable {
  ...
  //Constructor
  //--------------------------
  public Servidor(String video_file_name, NodeManager nm) {
    this.nm = nm;
          ...
          try {
            ...
            if(!flows.isEmpty())
              for(String destination : flows.get(Constants.SERVER_ID)){
                /* TODO: get client IP address from structure of IP, with destination string */
                // nm.get_client_IP(destination); // something like this
  ...
  public void run() {
    ...
    if (f.exists()) {
      ...
      Servidor s = new Servidor(VideoFileName, nm);
      //show GUI: (opcional!)
      //s.pack();
      //s.setVisible(true);
    } else
